...
def get_bucket_policies(bucket_name, event):
    # Get a list of S3 buckets and their bucket policies and evaluate each for compliance
    compliance_results = {}
    s3_client = get_client('s3', event)
    compliance = 'NON_COMPLIANT'
    annotate = 'There are no S3 buckets in this account.'
    try:
        policy = s3_client.get_bucket_policy(Bucket=bucket_name)
        annotate = None
        compliance = 'COMPLIANT'
    except s3_client.exceptions.from_code('NoSuchBucketPolicy'):
        annotate = f'There is no policy attached to the {bucket_name} bucket.'
    compliance_results[f'{bucket_name}'] = [annotate, compliance]
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    if not s3_response['Buckets']:
        evaluations.append(build_evaluation(event['accountId'], 'NOT_APPLICABLE', event))
        return evaluations
    for bucket in s3_response['Buckets']:
        evaluation_payload = get_bucket_policies(bucket['Name'], event)
        evaluations.append(build_evaluation(bucket['Name'], evaluation_payload[bucket['Name']][1], event, annotation=evaluation_payload[bucket['Name']][0]))
    return evaluations
